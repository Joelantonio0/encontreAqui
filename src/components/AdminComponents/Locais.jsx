...
function Locais() {
  ...
  const handleCadastrarCategoria = (e) => {
    ...
    fetch("http://localhost:5000/novo_local_perda", {
      method: "POST",
      headers: {
  ...
  const handleEliminarCategoria = (pk_local) => {
    fetch("http://localhost:5000/listar_locais_perda", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify({ pk_local: pk_local }),
    })
      .then((response) => response.json())
      .then((data) => {
        console.log(data);
  ...
  const handleMostrarOpcao = (pk_local) => {
    setCategoriaID(pk_local);
    setVisible2(true);
            ...
            <Column
              body={(rowData) => (
                <Button
                  ...
                  onClick={() => {
                    handleMostrarOpcao(rowData.pk_local);
                  }}
                />
              )}
              header="Eliminar"
